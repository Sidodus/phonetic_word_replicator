const FetchCtrlZh = (function () {


    // Public Methods
    return {
        fetchDico: async function () {
                let response = await fetch('wiktionary-json/zh.dict.v0.json');
...
const ItemCtrlZh = (function () {
    ...
    return {
        ...
        starter: function () {
            ...
            let phoneticsSoundType = localStorage.getItem('Phonetics Sound(zh)');

            if (phoneticsSoundType === null || phoneticsSoundType === '') {
                localStorage.setItem('Phonetics Sound(zh)', 'IPA')
            }

            // Set Display / App Based On Phonetics Sound
            if (localStorage.getItem('Phonetics Sound(zh)') === 'IPA') {
                AppCtrlZh.IPASoundInit()
            } else {
                AppCtrlZh.xSampaSoundInit();
            }

            // Load Current Attempted Questions Back To UI If Any
            if (localStorage.getItem('Current Attempted Questions(zh)')) {
                let displayCurrentAttemptedQuestions = JSON.parse(localStorage.getItem('Current Attempted Questions(zh)'));
                ...
                displayCurrentAttemptedQuestions.forEach(function (current) {
                    ...
                    UICtrlZh.getUISelector().currentQuestions.appendChild(liElement);
                }) // END OF forEach
             ...
             */
            if (sessionStorage.getItem('Previous Pronunciation(zh)') === null && sessionStorage.getItem('Previous Word(zh)') === null) {
                return;
            } else {
                sessionStorage.removeItem('Previous Pronunciation(zh)');
                sessionStorage.removeItem('Previous Word(zh)');
            } // **************************************************************
...
const AppCtrlZh = (function (fetchCtrlZh, itemCtrlZh, uiCtrlZh) {

    const UISelectors = uiCtrlZh.getUISelector();

    // Event Listeners
    let allEvents = function () {
        /* NOTE:
         ** Next Btn Event Is Located In AppCtrlZh.captureDico()
         */
    ...
    const IPASound = function () {
        ...
        let CurrentPronunciation = sessionStorage.getItem('Current Pronunciation(zh)');
        let CurrentWord = sessionStorage.getItem('Current Word(zh)');
        let EquivalentPronunciation = sessionStorage.getItem('Equivalent Pronunciation(zh)');
        ...
        UISelectors.ipaXSampaDictionary.textContent = '/ㄗ ˋ  ㄉ ㄧ ㄢ/';
        UISelectors.dictionary.textContent = '字典';

        // Change Pronunciation On Click
        if (phoneticsSoundType === 'IPA') {
            // Retain Current UI Pronunciation Display
            UISelectors.vowelSound.textContent = sessionStorage.getItem('Current Pronunciation(zh)');
            //            console.log('Already In IPA')
        } else {
            // Change Current UI Pronunciation Display
            UISelectors.vowelSound.textContent = sessionStorage.getItem('Equivalent Pronunciation(zh)');
            //            console.log('Changed To IPA');

            // Change Current & Equivalent State
            sessionStorage.setItem('Current Pronunciation(zh)', EquivalentPronunciation)
            sessionStorage.setItem('Equivalent Pronunciation(zh)', CurrentPronunciation)
        }
    } // END OF IPASound()
    ...
    const xSampaSound = function () {
        ...
        localStorage.setItem('Phonetics Sound(zh)', 'X-SAMPA');
        ...
        UISelectors.ipaXSampaDictionary.textContent = ''; // Change To XSAMPA (zh)
        UISelectors.dictionary.textContent = '字典';
    ...
    return {
        fetchedDico: function () {
            fetchCtrlZh.fetchDico()
                .then(function (data) {
                    //                console.log(data);

                    itemCtrlZh.starter();

                    let returnedData = itemCtrlZh.start(data);
                    //                                        console.log(returnedData);

                    AppCtrlZh.initCapturedDico(returnedData);
                    //                    AppCtrlZh.allEvents(returnedData)
                })
                .catch(function (err) {
                    console.error(err)
                }),
        ...
        countDownTimer: function (currentWord) {
            ...
            document.getElementById('correct-answer').innerHTML = `<p>正確答案： <span style="color: yellow; font-size: 1.4em"> ${currentWord} </span></p>`

            setTimeout(function () {
                // CountDown Timer To Dela Screen For 5Sec
                let timeLeft = 5;
                let downloadTimer = setInterval(function () {
                    document.getElementById('timer').innerHTML = '更改問題 ' + timeLeft + ' 秒';
...
<span class="init2-sound">/口語/</span>
<span class="init2-word badge badge-dark badge-pill" style="color: darkgoldenrod">字</span>
</li>
`
        ...
        calculatePlayRatio: function (btnClickType) {
            ...
            if (btnClickType === 'endPlayBtn') {
                ...
                correctOn1stAttempt = Number(localStorage.getItem('Current Correct Word On 1st Attempt(zh)'));
                correctOn2ndAttempt = Number(localStorage.getItem('Current Correct Word On 2nd Attempt(zh)'));
                correctOn3rdAttempt = Number(localStorage.getItem('Current Correct Word On 3rd Attempt(zh)'));
                wrongAfter3rdAttempt = Number(localStorage.getItem('Current Wrong Answer On 3rd Attempt(zh)'));

                deductedHintPoint = Number(localStorage.getItem('Deducted Hint Point(zh)'));
                ...
                // Calculate & Display Current Statistics
                correctOn1stAttempt = Number(localStorage.getItem('Correct Word On 1st Attempt(zh)'));
                correctOn2ndAttempt = Number(localStorage.getItem('Correct Word On 2nd Attempt(zh)'));
                correctOn3rdAttempt = Number(localStorage.getItem('Correct Word On 3rd Attempt(zh)'));
                wrongAfter3rdAttempt = Number(localStorage.getItem('Wrong Word(zh)'));
        ...
        displayToModal: function (liText, btnFunctionType) {
            ...
            if (liText !== null) {
                ...
                liText.forEach(function (text) {
                    ...
                    btnLink.setAttribute('href', `https://zh.wiktionary.org/wiki/${wordToLowerCase}`);
        ...
        capturedDico: function (iteratedData, iteratedDataFile, className) {
            ...
            if (iteratedDataFile === undefined) {
                ...
                if (phoneticsSoundType === 'IPA') {
                    ...
                    sessionStorage.setItem('Current Pronunciation(zh)', iteratedDataFilePronunciationIPA);
                    sessionStorage.setItem('Current Word(zh)', iteratedDataFileTitle);

                    // Save IPA Equivalent Pronunnciation To Session Storage
                    sessionStorage.setItem('Equivalent Pronunciation(zh)', iteratedDataFilePronunciationXSAMPA);

                    /*
                     * Delay Current Question Display In UI
                     * Previous Pronounciation Always Removed On Page Load From ItemCtrlZh.start()
                     */
                    if (sessionStorage.getItem('Previous Pronunciation(zh)') === null && sessionStorage.getItem('Previous Word(zh)') === null) {
                        ...
                        let liElement = document.createElement('li');
                        liElement.className = 'list-group-item d-flex justify-content-between align-items-center';
                        let liText = document.createTextNode(sessionStorage.getItem('Previous Pronunciation(zh)'));
                        ...
                        let spanText = document.createTextNode(sessionStorage.getItem('Previous Word(zh)'));

                        // Create A Button To Link Words To Wiktionary For Defination/Meaning
                        let btnLink = document.createElement('a');
                        btnLink.setAttribute('target', '_blank');

                        let btnLinkSpan = sessionStorage.getItem('Previous Word(zh)');
                        btnLinkSpan.toLowerCase();

                        btnLink.setAttribute('href', `https://zh.wiktionary.org/wiki/${btnLinkSpan}`);
                        ...
                        let currentAttemptedPhonics = sessionStorage.getItem('Previous Pronunciation(zh)')
                        let currentAttemptedWord = sessionStorage.getItem('Previous Word(zh)');
                        ...
                        if (localStorage.getItem('Current Attempted Questions(zh)') === null) {
                            currentAttemptedQuestions = [];

                        } else {
                            currentAttemptedQuestions = JSON.parse(localStorage.getItem('Current Attempted Questions(zh)'))

                        }

                        currentAttemptedQuestions.push(currentAttemptedPhoniceWord);
                        localStorage.setItem('Current Attempted Questions(zh)', JSON.stringify(currentAttemptedQuestions))

                        // Save All Attempted Questions To Local Storage
                        let previousWord = sessionStorage.getItem('Previous Word(zh)');
                        let previousIpa = sessionStorage.getItem('Previous Pronunciation(zh)');
                        let previousXSampa = sessionStorage.getItem('Previous Equivalent Pronunciation(zh)');
                        ...
                        if (localStorage.getItem('All Attempted Questions(zh)') === null) {
                            allAttemptedQuestions = [];

                        } else {
                            allAttemptedQuestions = JSON.parse(localStorage.getItem('All Attempted Questions(zh)'))

                        }

                        allAttemptedQuestions.push(storeAttemptedQuestion);
                        localStorage.setItem('All Attempted Questions(zh)', JSON.stringify(allAttemptedQuestions));
                    }

                    ////////////////////////////////////////////////////////////////
                    // Delay Display Of Right Side Value By First Saving It To Session Storage As Previous
                    sessionStorage.setItem('Previous Pronunciation(zh)', iteratedDataFilePronunciationIPA);
                    sessionStorage.setItem('Previous Equivalent Pronunciation(zh)', iteratedDataFilePronunciationXSAMPA);
                    sessionStorage.setItem('Previous Word(zh)', iteratedDataFileTitle);
                    ...
                    // Save Current Pronunnciation / Word To Session Storage
                    sessionStorage.setItem('Current Pronunciation(zh)', iteratedDataFilePronunciationXSAMPA);
                    sessionStorage.setItem('Current Word(zh)', iteratedDataFileTitle);

                    // Save X-SAMPA Equivalent Pronunnciation To Session Storage
                    sessionStorage.setItem('Equivalent Pronunciation(zh)', iteratedDataFilePronunciationIPA);
                    ...
                    if (phoneticsSoundType === 'IPA') {
                        ...
                        sessionStorage.setItem('Previous Pronunciation(zh)', iteratedDataFilePronunciationXSAMPA);
                        sessionStorage.setItem('Previous Equivalent Pronunciation(zh)', iteratedDataFilePronunciationIPA);
                        sessionStorage.setItem('Previous Word(zh)', iteratedDataFileTitle);
                    }
                }
        ...
        initCapturedDico: function (returnedData) {
            ...
            let iteratedData = itemCtrlZh.iterationCounts(dicoFile);
            //            let phoneticsSoundType = localStorage.getItem('Phonetics Sound(zh)');

            //            console.log(returnedData.data);

            let iteratedDataFile = iteratedData.next().value;
            AppCtrlZh.capturedDico(iteratedData, iteratedDataFile);
            ...
            UISelectors.hintsBtn.addEventListener('click', function () {
                ...
                let showHints = confirm("使用提示選項（每3次）將算作1次損失：'，'繼續");
                ...
                if (showHints === false) {
                    ...
                    let hintsStorage = 0;
                    if (localStorage.getItem('Hints Count(zh)') === null) {
                        //                        console.log('No Hints In Storage(zh)')
                        localStorage.setItem('Hints Count(zh)', '1')
                    } else {
                        //                        console.log('Hints Available In Storage');
                        hintsStorage = Number(localStorage.getItem('Hints Count(zh)'))
                        hintsStorage += 1;

                        localStorage.setItem('Hints Count(zh)', JSON.parse(hintsStorage))

                        // Deduct  Point After Every 3 Hints
                        let deductedHintPoint = 1;
                        if (Number(localStorage.getItem('Hints Count(zh)')) === 3) {
                            // Delete Point
                            if (localStorage.getItem('Deducted Hint Point(zh)') === null) {
                                localStorage.setItem('Deducted Hint Point(zh)', '1')
                            } else {
                                deductedHintPoint = Number(localStorage.getItem('Deducted Hint Point(zh)'));
                                deductedHintPoint += 1
                            }
                            localStorage.setItem('Deducted Hint Point(zh)', JSON.parse(deductedHintPoint))

                            // Delete Hints Count From Storage On Reaching 3 Count
                            localStorage.removeItem('Hints Count(zh)');
                        }
                    ...
                    if (arr.length === 1) {
                        ...
                        document.getElementById('display-hints').textContent = `SORRY: No Hints Available For ${arr.length} Group Word`
                    } else {
            ...
            UISelectors.selectLanguage.addEventListener('click', function () {
                ...
                // Modal Titlle
                document.querySelector('.modal-title').innerHTML = `選擇你的語言`;
...
`
                ...
                document.getElementById('chinese').addEventListener('click', function () {
                    ...
                    if (localStorage.getItem('PWR Language') === 'Chinese') {
                        // Close Modal
            ...
            UISelectors.endPlayBtn.addEventListener('click', function () {
                ...
                if (localStorage.getItem('Current Attempted Questions(zh)') === null) {
                    ...
                    let endPlay = confirm('您即將結束練習：CONFIRM');
                    ...
                    if (endPlay === false) {
                        ...
                        let totalCurrentPassedAttempt = AppCtrlZh.calculatePlayRatio('endPlayBtn').totalCurrentPassedAttempt;

                        // Modal Titlle
                        document.querySelector('.modal-title').innerHTML = `回答的問題： <small style="color: darkgoldenrod"> ${totalCurrentPassedAttempt}% 準確性 </small>`;

                        // Get Current Questions From Session Storage
                        let liText = JSON.parse(localStorage.getItem('Current Attempted Questions(zh)'));

                        // Display Current Questions To Modal Body
                        AppCtrlZh.displayToModal(liText, 'endPlayBtn');
                    }
                }

                // Clear Current Questions + Calculation From Session Storage
                localStorage.removeItem('Current Correct Word On 1st Attempt(zh)');
                localStorage.removeItem('Current Correct Word On 2nd Attempt(zh)');
                localStorage.removeItem('Current Correct Word On 3rd Attempt(zh)');
                localStorage.removeItem('Current Wrong Answer On 3rd Attempt(zh)')
                localStorage.removeItem('Current Attempted Questions(zh)');
            ...
            UISelectors.winRateBtn.addEventListener('click', function () {
                ...
                // Calculate & Display Current Statistics
                let calculatePlayRatio = AppCtrlZh.calculatePlayRatio();
                ...
                // Modal Titlle
                document.querySelector('.modal-title').innerHTML = `分析所有回答的問題`;
...
`
            ...
            UISelectors.historyBtn.addEventListener('click', function () {
                ...
                // Calculate & Display Current Statistics
                let calculatePlayRatio = AppCtrlZh.calculatePlayRatio('historyBtn');

                let totalAttemptsRatio = calculatePlayRatio.totalAttemptsRatio

                // Modal Titlle
                document.querySelector('.modal-title').innerHTML = `所有回答的問題： <small style="color: darkgoldenrod"> ${totalAttemptsRatio}% 準確性 </small>`;


                // Get Current Questions From Session Storage
                let liText = JSON.parse(localStorage.getItem('All Attempted Questions(zh)'));


                // Display Current Questions To Modal Body
                AppCtrlZh.displayToModal(liText, 'historyBtn');
            ...
            UISelectors.nextBtn.addEventListener('click', function () {
                ...
                let currentWord = sessionStorage.getItem('Current Word(zh)').toLowerCase();
                ...
                if (UISelectors.inputWord.value.toLowerCase() === '' || UISelectors.inputWord.value.toLowerCase() === ' ') {
                    ...
                    if (localStorage.getItem('Correct Word On 1st Attempt(zh)') === null) {
                        localStorage.setItem('Correct Word On 1st Attempt(zh)', '1');

                    } else {
                        let currentWinOn1 = Number(localStorage.getItem('Correct Word On 1st Attempt(zh)'))
                        currentWinOn1 += 1;
                        localStorage.setItem('Correct Word On 1st Attempt(zh)', JSON.parse(currentWinOn1))
                    }

                    // Save Current Winning Attempt To Session Storage
                    // For End Game Calculation
                    if (localStorage.getItem('Current Correct Word On 1st Attempt(zh)') === null) {
                        localStorage.setItem('Current Correct Word On 1st Attempt(zh)', '1');

                    } else {
                        let currentCorrectWinOn1 = Number(localStorage.getItem('Current Correct Word On 1st Attempt(zh)'))
                        currentCorrectWinOn1 += 1;
                        localStorage.setItem('Current Correct Word On 1st Attempt(zh)', JSON.parse(currentCorrectWinOn1))
                    }
                    ...
                    let iteratedDataFile = iteratedData.next().value;
                    let capturedData = AppCtrlZh.capturedDico(iteratedData, iteratedDataFile, className);
                    //                                        console.log('WORD:', sessionStorage.getItem('Current Word(zh)'), 'IPA:', sessionStorage.getItem('Current Pronunciation(zh)'))
            ...
            UISelectors.tryAgainBtn.addEventListener('click', function () {
                ...
                if (UISelectors.inputWord.value.toLowerCase() === '' || UISelectors.inputWord.value.toLowerCase() === ' ') {
                    ...
                    // Save Winning Rate On First Attempt To Local Storage
                    if (localStorage.getItem('Correct Word On 2nd Attempt(zh)') === null) {
                        localStorage.setItem('Correct Word On 2nd Attempt(zh)', '1');

                    } else {
                        let currentWinOn2 = Number(localStorage.getItem('Correct Word On 2nd Attempt(zh)'))
                        currentWinOn2 += 1;
                        localStorage.setItem('Correct Word On 2nd Attempt(zh)', JSON.parse(currentWinOn2))
                    }

                    // Save Current Winning Attempt To Session Storage
                    // For End Game Calculation
                    if (localStorage.getItem('Current Correct Word On 2nd Attempt(zh)') === null) {
                        localStorage.setItem('Current Correct Word On 2nd Attempt(zh)', '1');

                    } else {
                        let currentCorrectWinOn2 = Number(localStorage.getItem('Current Correct Word On 2nd Attempt(zh)'))
                        currentCorrectWinOn2 += 1;
                        localStorage.setItem('Current Correct Word On 2nd Attempt(zh)', JSON.parse(currentCorrectWinOn2))
                    }
                    ...
                    let iteratedDataFile = iteratedData.next().value;
                    AppCtrlZh.capturedDico(iteratedData, iteratedDataFile, className);
            ...
            UISelectors.lastChanceBtn.addEventListener('click', function () {
                ...
                if (UISelectors.inputWord.value.toLowerCase() === '' || UISelectors.inputWord.value.toLowerCase() === ' ') {
                    ...
                    AppCtrlZh.countDownTimer(currentWord);
                    // Hide Button
                    UISelectors.lastChanceBtn.style.display = 'none';
                    ...
                    setTimeout(function () {
                        ...
                        if (localStorage.getItem('Wrong Word(zh)') === null) {
                            localStorage.setItem('Wrong Word(zh)', '1');

                        } else {
                            let wrongWord = Number(localStorage.getItem('Wrong Word(zh)'))
                            wrongWord += 1;
                            localStorage.setItem('Wrong Word(zh)', JSON.parse(wrongWord))
                        }

                        // Save Current Wrong Attempt To Session Storage
                        // For End Game Calculation
                        if (localStorage.getItem('Current Wrong Answer On 3rd Attempt(zh)') === null) {
                            localStorage.setItem('Current Wrong Answer On 3rd Attempt(zh)', '1');

                        } else {
                            let currentWrongAnsOn3 = Number(localStorage.getItem('Current Wrong Answer On 3rd Attempt(zh)'))
                            currentWrongAnsOn3 += 1;
                            localStorage.setItem('Current Wrong Answer On 3rd Attempt(zh)', JSON.parse(currentWrongAnsOn3))
                        }
                    ...
                    // Save Winning Rate On First Attempt To Local Storage
                    if (localStorage.getItem('Correct Word On 3rd Attempt(zh)') === null) {
                        localStorage.setItem('Correct Word On 3rd Attempt(zh)', '1');

                    } else {
                        let currentWinOn3 = Number(localStorage.getItem('Correct Word On 3rd Attempt(zh)'))
                        currentWinOn3 += 1;
                        localStorage.setItem('Correct Word On 3rd Attempt(zh)', JSON.parse(currentWinOn3))
                    }

                    // Save Current Winning Attempt To Session Storage
                    // For End Game Calculation
                    if (localStorage.getItem('Current Correct Word On 3rd Attempt(zh)') === null) {
                        localStorage.setItem('Current Correct Word On 3rd Attempt(zh)', '1');

                    } else {
                        let currentCorrectWinOn3 = Number(localStorage.getItem('Current Correct Word On 3rd Attempt(zh)'))
                        currentCorrectWinOn3 += 1;
                        localStorage.setItem('Current Correct Word On 3rd Attempt(zh)', JSON.parse(currentCorrectWinOn3))
                    }
...
})(FetchCtrlZh, ItemCtrlZh, UICtrlZh); // END OF AppCtrlZh()
